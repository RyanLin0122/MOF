...
// ============================================================================
// CCA — restored from mofclient.c (0x00524C8C .. 0x00526E00)
// ============================================================================
...
// this port use named access (m_pCCA->m_fPosX) — search the repo to confirm.
// -----------------------------------------------------------------------------
...
// we preserve that default here — every slot falls through to case 0 at load,
...
// =============================================================================
// Process() — top-level per-frame tick (0x005251A0)
// =============================================================================
bool CCA::Process()
{
    if (!g_bProcessInit)
    ...
    // a pointer-sized int, which is meaningless here — we simulate the same
...
// =============================================================================
// Process(GameImage*) — per-layer draw-list rebuild (0x00525290)
//
    ...
    for (int slot = 0; slot < 23; ++slot)
    {
        ...
        for (int e = 0; e < nEntry; ++e)
        {
            ...
            if (!pGI || !pGI->m_pGIData)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        // (other fallback cases unused — byte_525EB4 is weak/zero)
                    }
                }
            }
            ...
            if (pBack->m_pGIData)
            {
                ...
                // +28, +32.  We don't reimplement that walker here — instead
...
bool CCA::Draw(int viewport)
{
    ...
    if (m_pEffectBefore && !m_nTransportActive)
    {
        ...
        if (pEff)
        {
            ...
            // We cannot dispatch through an unknown vtable slot here — instead
...
// =============================================================================
// SetItemID (0x005261C0) — dispatches to LayerPutOn / LayerPutOff / SetLayerByItemKind
// =============================================================================
void CCA::SetItemID(uint16_t itemID, uint8_t sex, int mode, int hairIdx, int faceIdx, uint8_t slotIndex)
{
    ...
    if (itemKind == 13)
    {
        // SUIT — spread across coat/pants/shoes/hand/suit slots.
...
// =============================================================================
// LayerPutOff (0x005269D0) — clears layer slots for a given kind
// =============================================================================
