...
// 建議保留為可調整的常數（目前呼叫端以 +8 位移抓 VB 指標）
#ifndef VBDATA_VB_OFFSET
#   define VBDATA_VB_OFFSET 8
#endif

bool CDeviceResetManager::IsDeviceReady() const {
	return Device && Device->TestCooperativeLevel() == D3D_OK;
}

IDirect3DVertexBuffer9* CDeviceResetManager::GetVertexBuffer(const VertexBufferData* h) const {
	return h ? h->pVertexBuffer : nullptr;
}

bool CDeviceResetManager::UpdateVertexBuffer(const VertexBufferData* h, const void* src, size_t bytes, DWORD lockFlags) const {
	if (!src || bytes == 0) return false;
	if (!IsDeviceReady())   return false;
	IDirect3DVertexBuffer9* vb = GetVertexBuffer(h);
	if (!vb) return false;
	void* dst = nullptr;
	const HRESULT hr = vb->Lock(0, 0, &dst, lockFlags);
	if (FAILED(hr) || !dst) return false;
