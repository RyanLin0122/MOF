...
/**
 * @class CEffect_Field_HealNumber
 * @brief 負責顯示治療或恢復數字的程序化動畫特效。
 */
class CEffect_Field_HealNumber : public CEffectBase {
public:
    CEffect_Field_HealNumber();
    virtual ~CEffect_Field_HealNumber();
    ...
    // --- 專有函式 ---
    /// @brief 設定特效。
    /// @param healValue 顯示的治療數值。
    /// @param x 特效中心點的 X 座標。
    /// @param y 特效中心點的 Y 座標。
    void SetEffect(int healValue, float x, float y);

private:
    static const int MAX_DIGITS = 10; // 最多顯示10位數

    // --- 成員變數 (根據 Effectall.c @ 0x005356F0 推斷) ---
