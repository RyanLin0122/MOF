...
- 目前已完成的程式碼共 820 個標頭/原始檔（不含 Test 與 fmod），約 138,000 行，其中 `src/` 下有 404 個 `.cpp` 實作檔。

### 前端開發進度: 56%
### 剩餘未實作:
- UI: 36%
- 主遊戲邏輯: 8%（NPC、Skill System、Circle、部分Info）。
...
- 集中宣告並實體化主要全域物件，例如：
  ...
  - `g_clPetSystem`、`g_clMoneySystem`、`g_clClientPetKindInfo`：寵物與經濟系統（pet kind 資料透過 `g_clClientPetKindInfo.PetKindInfo()` 取用）。
...
- `cltMyCharData` 為當前玩家角色的完整初始化資料（從 `Logic` 移入此模組，與其他角色相關類別集中管理）。
...
這些類別通常對應「某種 KindInfo / Info 檔案」，由其他系統查表使用，而不是直接承擔畫面或流程控制。新增的 `cltMapAreaKindInfo`、`cltMapCollisonInfo` 補齊了地圖區域與碰撞資料的讀取支援，搭配 `Logic/` 下的地圖系統使用。
...
- `Map`：地圖資料與場景層級邏輯。
- `cltMap_Climate`、`CMap_Item_Climate`、`CItemClimateUnit`：地圖氣候/物件氣候資料管理與單元。
- `cltMap_WeddingFlower`：婚禮地圖花卉效果。
...
- `cltLogicCompatibility`：相容層墊片，目前承接 `cltShopInfo` 等尚未完整實作之類別的最小可編譯實作（如 `GetShopInfoByID`、`GetPackageItemPrice`）。
- `CMatchTeamInfo` / `CMatchUserInfo`：PvP / Match 隊伍容器與單一隊員 wrapper（`CMatchUserInfo` 為 4-byte account ID；`CMatchTeamInfo` 內含 7 格固定大小的 user 陣列，提供 `AddCharacter` / `DelCharacter` / `IsExistEmptyCharSlot` 等 slot 管理）。
- `CCircleValueCalculator`：公會（Circle）相關純計算工具，把迷你遊戲種類與訓練成果差距轉成系統內部的公會貢獻分數（`GetCircleMiniGameValue` / `GetCircleTrainingValue`）。
- `CDeleteCashItem` / `CDeleteCashItemManager`：商城道具刪除通知。`CDeleteCashItem` 內部以 `std::vector<strDeleteItem>` 暫存待通知道具，支援彈出 7 天內到期警示；`CDeleteCashItemManager` 為 owner，提供 `InitDeleteCashItem` / `ClearDeleteCashItem`。
- `MapFileInfoHead`：`.map` 檔頭描述子，僅作為 binary-equivalent 的型別記號存在（實際資源釋放已由 `Map` 類別自身的 destructor 處理）。

`Logic` 是支撐玩法運作的基礎行為層。注意：`cltMyCharData` 已從本目錄移至 `Character/`；`cltShopInfo` 仍以標頭定義為主，方法本體放在 `cltLogicCompatibility.cpp`。
...
- `cltMoF_MiniGame_Mgr`：小遊戲整體管理器，串接基底類別與全域生命週期（建立/切換/釋放各小遊戲實例）。
...
- `cltMini_Bow` / `cltMini_Bow_2`：弓手小遊戲（兩個版本），搭配 `cltBow2_Char`、`cltBow2_Spear` 處理 v2 的角色與長矛標靶元件。
- `cltMini_Magic` / `cltMini_Magic_2`：魔法小遊戲（兩個版本），搭配 `cltMagic_Box`、`cltMagic_Target` 處理魔法盒與目標元件。
...
每個小遊戲都繼承 `cltMoF_BaseMiniGame`，以 `Poll()` 虛擬函式驅動每幀更新。劍士、驅魔、弓手、魔法四大職業的 v1 / v2 版本都已備齊，加上醫師（`CHospital` / `CMedical`）線，形成五個職業系列的小遊戲集合。
...
- Lobby 背景：`MainTitleBG`（繼承 `CCAEffect`，載入 `Ui_CharBG.ea` 並播放第 0 段動畫），以及 `LobbyBackground` namespace（提供 `Ensure` / `Process` / `Draw` / `Free` 入口，等價於原 binary 中 `GsInit*` / `GsSelectChannel_*` 對 `g_pLobbyBackground` 的操作）。
...
- `comp.h`：壓縮/解壓縮工具。
- `CA_CACHE_DATA`：CA / CCA 模組共用的 GameImage 快取節點（繼承 `GameImage`、總大小 516 bytes，於 `CA_GI_Cache` 內成串使用）。
- `GIHEADINFO`：`.gi` 檔頭描述子，與 `ImageResource` 共用前段欄位，destructor 會釋放影格資訊與像素資料兩個動態 buffer。
...
- **世界地圖**：`CUIWorldMap`。
- **主要 UI 場景控制器（目前多為待補的最小型別宣告 / stub）**：`CUIBasic`（遊戲主畫面 HUD）、`CUIMenu`（主選單視窗）、`CUITradeUser`（玩家交易視窗）、`CUICircle`（公會 / 同好會社群介面）；這幾個視窗主要是給 `cltChattingMgr`、`CShortKey` 等已還原的呼叫端提供型別與 null-safe 入口，待後續完整實作。
- **資料介接**：`CInterfaceDataCommunity` 透過全域 `g_pInterfaceDataCommunity` 對外，承接組隊/公會邀請、踢人、離開、開啟玩家資料等聊天命令；目前以 stub 形式存在，僅維持 `m_iWaitCallFlag` 與少量 hooks，足以讓聊天/社群命令路徑安全運作。
- **整體管理**：`CUIManager` 彙整各 UI 子元件，`CUITutorial` 專注於教學引導畫面，`CUIBase` 提供 UI 基底。
- **共用資料節點**：
  - `UIMSG`：6 × DWORD 的 UI event 資料包，被 `CUI*::OnEvent_*` 系列當作參數陣列（含 event ID、來源 `CControlBase*`、4 個 payload 整數）。
  - `stNPCQuestData`：`CInterfaceData` 內嵌欄位，記錄 NPC 對話過程的任務 ID / 狀態 / 步驟 / flag。
  - `stQuestMark`：世界地圖 / DivideMap 任務標記節點，由 `CUIWorldMap` / `CUIDivideMap` 以 `std::map<uint16_t, stQuestMark*>` 持有。
  - `strUIHelperMenu`（搭配 `strUIHelperBallon`）：`CUIHelper` 用 vector 管理多個 helper window 的選單與氣球結構。
  - `tNPCKind`：`CUINPC::GetNPCKind` 以 `qmemcpy` 拷貝出來的 64-byte NPC 種類描述，目前只還原 `GetNPCID()` 介面。
...
- `CCmdLine`：保存 `WinMain` 傳入的 `lpCmdLine`，供之後 `ShellExecute` 啟動 Recharge.exe 使用（內部即一個 1024-byte 的字串緩衝）。
- `CLog`：三向（stdout / 檔案 / 自繪 LISTBOX）日誌輸出器，透過 `Singleton<CLog>::m_singleton` 提供全域單例，以 flag bitmask 控制輸出目的地。
- `CLogWriter`：thread-safe append-only 單檔 logger，內含 `CRITICAL_SECTION`，提供帶檔/行號（assert 用）與帶日期前綴兩種 `PrintF` overload。
- `cltRegistry`：讀取 `HKEY_CURRENT_USER` / `HKEY_LOCAL_MACHINE` 下安裝資訊（"Path"、"GameVersion"），並可把 `"AA.BB.CC.DD"` 版本字串壓成單一整數。
...
- `DCTAbuseWordManager`：髒話/敏感詞過濾管理（聊天送出前的字串檢查）。
